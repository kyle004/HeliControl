...
namespace Oxide.Plugins {
  ...
  public class HeliControl: RustPlugin {
    ...
    private void FlyHelicopterTo(BaseEntity miniCopterEntity, Vector3 targetPosition, float targetAltitude, BasePlayer player, float gainAltitude = 10 f) {
      if (miniCopterEntity == null) {
        Puts("MiniCopter is null!");
        return;
      }
      ...
      int numberOfRepetitions = Mathf.CeilToInt(Vector3.Distance(miniCopterEntity.transform.position, targetPosition) / (flightSpeed * 0.1 f));

      // Gain altitude and rotate the helicopter towards the target position
      GainAltitude(helicopterRigidbody, 10 f);
      RotateHelicopterTowards(miniCopterEntity, targetPosition);

      Quaternion newRotation = Quaternion.Euler(
        miniCopterEntity.transform.rotation.eulerAngles.x + pitchAngle,
        miniCopterEntity.transform.rotation.eulerAngles.y + yawAngle,
        miniCopterEntity.transform.rotation.eulerAngles.z + rollAngle
      );
      miniCopterEntity.transform.rotation = Quaternion.Slerp(miniCopterEntity.transform.rotation, newRotation, Time.deltaTime * rotationSpeed);

      // Add a short delay before flying towards the target
      timer.Once(2.0 f, () => {
        Timer flyTimer = null;
        flyTimer = timer.Repeat(0.1 f, numberOfRepetitions, () => {
          ...
          if (distanceToTarget > 0.1 f) {
            ...
            ApplyRotationDuringFlight(miniCopterEntity, direction);
            ...
            if (currentAltitude < targetAltitude) {
              ...
              helicopterRigidbody.AddForce(forceDirection * forceMagnitude, ForceMode.Acceleration);
            } else if (currentAltitude - currentPosition.y < 1.0 f) {
    ...
    private void RotateHelicopterTowards(BaseEntity miniCopterEntity, Vector3 targetPosition) {
      ...
      // Apply pitch, yaw, and roll adjustments
      var adjustedRotation = lookRotation * Quaternion.Euler(pitchAngle, yawAngle, rollAngle);
    ...
    private void ApplyRotationDuringFlight(BaseEntity miniCopterEntity, Vector3 direction) {
      if (miniCopterEntity == null) {
        Puts("MiniCopter is null!");
        return;
      }
      ...
      // Calculate desired forward, right, and up directions
      Vector3 desiredForward = (targetPosition - miniCopterEntity.transform.position).normalized;
      Vector3 desiredRight = Vector3.Cross(desiredForward, miniCopterEntity.transform.up).normalized;
      Vector3 desiredUp = Vector3.Cross(desiredRight, desiredForward).normalized;

      // Calculate pitch, yaw, and roll angles
      pitchAngle = Vector3.SignedAngle(miniCopterEntity.transform.forward, desiredForward, miniCopterEntity.transform.right);
      yawAngle = Vector3.SignedAngle(miniCopterEntity.transform.forward, desiredForward, miniCopterEntity.transform.up);
      rollAngle = Vector3.SignedAngle(miniCopterEntity.transform.right, desiredRight, miniCopterEntity.transform.forward);
